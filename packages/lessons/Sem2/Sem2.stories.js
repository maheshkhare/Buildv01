...
//   const fileName = sessionStorage.getItem('jsonFile') || 'lesson1';
//   const questionCount = parseInt(sessionStorage.getItem('questionCount')) || 0;
//   const levelRange = sessionStorage.getItem('Exe_Number'); // e.g. '1'

//   let levelStart = 0, levelEnd = 0;
//   if (levelRange) {
//     const parts = levelRange.split('-').map(Number);
//     levelStart = parts[0];
//     levelEnd = parts[1] || levelStart; // support single level too
//   }
...
//     // ✅ Filter instructionSets by level
//     if (data.instructionSets && levelRange) {
//       filteredData = {
//         ...data,
//         instructionSets: data.instructionSets.filter(set => {
//           const lvl = Number(set.level);
//           return lvl >= levelStart && lvl <= levelEnd;
//         })
//       };
...
//       limitedData = {
//         ...filteredData,
//         questions: filteredData.questions.slice(0, questionCount || filteredData.questions.length)
//       };
//     } else if (filteredData.instructionSets) {
//       // ✅ Keep whole groups intact, up to questionCount
//       let totalCount = 0;
//       let pickedGroups = [];

//       for (const group of filteredData.instructionSets) {
//         if (questionCount && (totalCount + group.questions.length) > questionCount) {
//           // ✅ Partial slice — only if you want partial groups
//           const remaining = questionCount - totalCount;
//           if (remaining > 0) {
//             pickedGroups.push({
//               instructionText: group.instructionText,
//               questions: group.questions.slice(0, remaining)
//             });
//           }
//           break;
//         } else {
...
//       limitedData = {
//         ...filteredData,
//         instructionSets: pickedGroups
//       };
//     }

//     this.lessonData = limitedData;
...
//   //   const fileName = sessionStorage.getItem('jsonFile') || 'lesson1';
//   //   const questionCount = parseInt(sessionStorage.getItem('questionCount')) || 0;
//   //   const levelRange = sessionStorage.getItem('Exe_Number'); // Example: '1-3'
...
//   //     // Slice logic if needed
//   //     let limitedData;

//   //     if (data.questions) {
//   //       limitedData = {
//   //         ...data,
//   //         questions: data.questions.slice(0, questionCount || data.questions.length)
//   //       };
//   //     } else if (data.instructionSets) {
//   //       let totalQuestions = [];
//   //       data.instructionSets.forEach(group => {
//   //         group.questions.forEach(q => {
//   //           totalQuestions.push({ ...q, _instructionText: group.instructionText });
//   //         });
//   //       });

//   //       const limitedQuestions = totalQuestions.slice(0, questionCount || totalQuestions.length);

//   //       const groupedMap = {};
//   //       limitedQuestions.forEach(q => {
//   //         if (!groupedMap[q._instructionText]) {
//   //           groupedMap[q._instructionText] = [];
//   //         }
//   //         groupedMap[q._instructionText].push(q);
//   //       });

//   //       limitedData = {
//   //         ...data,
//   //         instructionSets: Object.keys(groupedMap).map(text => ({
//   //           instructionText: text,
//   //           questions: groupedMap[text].map(({ _instructionText, ...rest }) => rest)
//   //         }))
//   //       };
//   //     }
...
Practice.story = {
  ...
  parameters: {
    docs: {
      storyDescription: `Lesson data is passed from external JSON file.`
    }
  }
};
