...
<div class="flex-container">
    <div class="grid grid-cols-1 -ml-10" style="border:0px; width: 100%; ">
        ...
        <div v-if="ImageNames" class="md:shrink-0 justify-center ml-auto mr-auto mt-1 shadow-lg">
            <div style="width:160px; height:80px;text-align:center;border: 1px solid black;">
                <img :src="getImgUrl(ImageNames)" style="width:155px; height:70px;" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 sm:flex mt-4 ml-auto mr-auto">
            <SVGImageButton v-for="(word, index) in commonNumArray.slice(0,8)" :key="index" :identifier="index" :class="{
      ...
      'bg-white-0': word.state === 'unselected'
                ...
                <button v-on:click="WordsAnswer(word.Answer, word.index)" class="bg-blue-100 w-40 hover:bg-yellow-500 text-black-700 font-semibold py-2 px-2 border border-black shadow-lg rounded">
                    <img :src="getImgUrl(word.Question)" style="width:155px; height:70px;" />
                </button>
            </SVGImageButton>
        </div>
