    ...
    <div v-else class="w-full">
      ...
      <div class="rows-12">
        <div class="content-container">
          ...
          <div>
            <SectionSem1Bottom :show-next-button="showNextButton" 
             
                 :show-previous-button="currentQuestionIndex > 0" :isReviewMode="isReviewMode" :reviewTableRows="reviewTableRows" :accept-input="canModifyAnswers" :columns="columns" :column-titles="columnTitles" :column-visibility="columnVisibility" :row-counts="rowCounts" :highlight="highlightBottom" :showBottom="showBottom" :Arrow_isShowing="Arrow_isShowing || isSingleColumnMode" @OnClicked_Col="handleColumnClick" @Click_NextButton="Click_NextButton" @Click_PreviousButton="goToPreviousQuestion"/>
...
async mounted() {
  ...
  //       this.completedSets = parsed.DetailedResults.map((result, idx) => {
  ...
  //       this.$nextTick(() => {
  //         const first = this.completedSets[0];
  //         if (first) {
  //           this.currentQuestionIndex = 0;
  //           this.columnTitles = first.questionTitles || [];
  //           this.columns = JSON.parse(JSON.stringify(first.columns));
  //           this.availableWords = [];
  //           this.placedWords = first.columns.flat().map(c => c.name).filter(n => n !== "");
  //           this.canModifyAnswers = false;
